...
def _f(df, *args):
    ...
    v_swap = swap.value(value_dt, curve, curve)
...
class InterestRateCurve(DiscountCurve):
    ...
    def _validate_inputs(self, ois_deposits: list[InterestRateDeposit], ois_fras, ois_swaps: list[InterestRateSwap]):
        ...
        if num_depos > 0:
            ...
            depo_start_dt = ois_deposits[0].effective_dt

            for depo in ois_deposits:

                if isinstance(depo, InterestRateDeposit) is False:
                    raise FinError("Deposit is not of type InterestRateDeposit")

                start_dt = depo.effective_dt
        ...
        if num_swaps > 0:
            ...
            for swap in ois_swaps:
                if isinstance(swap, InterestRateSwap) is False:
                    raise FinError("Swap is not of type InterestRateSwap")
    ...
    def _build_curve_using_1d_solver(self):
        ...
        for fra in self.used_fras:
            ...
            t_set = times_from_dates(fra.start_dt, self.value_dt, self.dc_type)
            t_mat = times_from_dates(fra.maturity_dt, self.value_dt, self.dc_type)
    ...
    def _build_curve_linear_swap_rate_interpolation(self):
        ...
        for dt in cpn_dts[1:]:
            swap_time = times_from_dates(dt, self.value_dt, self.dc_type)
        ...
        for i in range(start_index, num_flows):
            ...
            dt = cpn_dts[i]
            t_mat = times_from_dates(dt, self.value_dt, self.dc_type)
    ...
    def _check_refits(self, depo_tol, fra_tol, swap_tol):
        ...
        for swap in self.used_swaps:
            ...
            v = swap.value(swap.effective_dt, self, self)
            v = v / swap.notional
            if abs(v) > swap_tol:
                print(
                    "Swap with maturity "
