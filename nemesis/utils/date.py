...
class Date:
    ...
    def add_weeks(self, weeks: Union[list, int]):
        """Returns a new date that is a specified number of weeks after the Date.
        If weeks is an integer or float, you get back a single date. If weeks is a list,
        you get back a list of dates."""

        num_weeks = 1
        scalar_flag = False

        if isinstance(weeks, int) or isinstance(weeks, float):
            weeks_vector = [weeks]
            scalar_flag = True
        else:
            weeks_vector = weeks

        num_weeks = len(weeks_vector)

        date_list = []

        for i in range(0, num_weeks):
            week_count = weeks_vector[i]

            # If I get a float I check it has no decimal places
            if int(week_count) != week_count:
                raise FinError("Must only pass integers or float integers.")

            week_count = int(week_count)

            new_dt = self.add_days(week_count * 7)
